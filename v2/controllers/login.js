...
exports.initialiseSystemLoginFlow = async function (req, res) {
  ...
  const body = req.body
  try {
    if (propsMissingIn(body, requiredProps)) { throw new KYIBadRequestException({ message: 'All required properties not supplied' }) }

    debug('All properties exist')
    const validUsernameTypes = ['_id', 'instituteEmail', 'admissionNumber']
    if (!isValidValue(body.usernameType, validUsernameTypes)) { throw new KYIBadRequestException({ message: 'Only ' + validUsernameTypes.join(', ') + ' are allowed as usernames' }) }
    // find client and check if it is system --> Client.findOne
    // TODO: Add secret check here
    const client = await Client.findOne({ _id: body.clientId }) // , secret: body.secret, isSystem: true
    if (client) {
      debug('Client found')
      // check user exists --> User.findOne
      const user = await User.findOne({ [body.usernameType]: { $regex: new RegExp(body.username), $options: 'i' } })
      if (user) {
        // generate OTP, mail it, hash it, save it --> generateOTP, mailer.send, OTP.save
        const generated = await generateAndSaveOTP(user, client)
        debug('OTP generated')
        ...
        // respond with userId
        const resp = new BaseResponse({ userId: user._id }, httpCodes.OK, {})
...
exports.verifySystemOTP = async function (req, res) {
  ...
  try {
    ...
    const otpEntity = await OTP.findOne({ used: false, otpHash: body.otp, userId: body.userId, expires: { $gt: Date.now() } })
    if (otpEntity) {
      ...
      const token = generateAccessToken(user, client, true)

      // respond with access_token
      const resp = new BaseResponse({ accessToken: token, user }, httpCodes.OK, {})
      res.status(resp.status).json(resp.getOneResponse)
