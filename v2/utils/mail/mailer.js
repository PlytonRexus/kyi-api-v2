...
async function getMailParts (opts, template, variables, provider) {
  ...
  const email = provider === 'gmail' ? process.env.GMAIL_ID : process.env.SMTP_EMAIL_ID
...
const send = async function (template, opts, variables) {
  if (!(template instanceof Template)) { throw new KYIInternalServerException({ message: 'Invalid template' }) }
  try {
